...
ES_t LCD_enuWriteIntegerNumber(s32 Copy_s32Number)
{
	...
	{
		Local_u8OppositedNumber = Local_u8OppositedNumber * LCD_u8REMINDERNUMBER + Copy_s32Number % LCD_u8REMINDERNUMBER;
		Copy_s32Number /= LCD_u8REMINDERNUMBER;
	}

	while( Local_u8OppositedNumber != 0)
	{
		Local_u8CurrentDigit = Local_u8OppositedNumber % LCD_u8REMINDERNUMBER;
		LCD_enuWriteData( Local_u8CurrentDigit + 48 );
		Local_u8OppositedNumber /= LCD_u8REMINDERNUMBER ;
	}
